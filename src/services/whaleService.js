...
async function fetchBlockchairWhales() {
  return getOrFetch('whales_blockchair', async () => {
    ...
    return json.data.map(tx => {
      ...
      const valueUsd = valueBtc * 98000 // approximate BTC price
      return {
...
// Fetch large ETH transactions using CoinCap assets endpoint for volume data
async function fetchCoinCapVolume() {
  return getOrFetch('whales_coincap', async () => {
    ...
    // Convert volume data into whale movement approximations
    return json.data.map(asset => {
      ...
      return {
        ...
        value: formatValue(volume24h / 24), // hourly approximation
        chain: asset.symbol === 'SOL' ? 'solana' : 'ethereum',
        time: 'Recent',
        impact: isBullish ? 'bullish' : 'bearish',
      }
    })
  }, 60_000) // 60s cache for volume data
}

export async function fetchRecentWhaleMovements() {
  try {
